...
// ---- authenticated handshake (Phase 8.6): AUTHENTICATED post-quantum key agreement (port of auth_pq_keyexchange.py) ----
//
// 8.5 made the handshake CONFIDENTIAL against a passive recorder (classical DH ⊕ ML-KEM). It did NOT stop
// an ACTIVE man-in-the-middle, who could sit between the two parties and impersonate each to the other.
// 8.6 closes that: it adds AUTHENTICATION — proof of who you are talking to — and makes that proof, like
// the confidentiality, hybrid on BOTH axes so it survives a quantum break:
//   * STATIC triple-DH binding — the decades-studied discrete-log identity proof (mixed into the key, so
//     an impostor who can't do the static DH derives a different key), AND
//   * ML-DSA-65 signatures (FIPS 204) over the full transcript — a post-quantum identity proof.
// An attacker must defeat BOTH to impersonate. Like ml-kem, the ML-DSA primitive is NOT hand-rolled: it is
// RustCrypto's reviewed `ml-dsa`, proven (see the parity tests) to interoperate with Python's OpenSSL 3.5
// ML-DSA-65 byte-for-byte. We invent only the transcript binding + the combiner.
//
// Determinism — the SAME discipline as 8.5: the caller owns ALL randomness. Every keypair is regenerated
// from an explicit seed (the 32-byte ML-DSA seed; the DH exponents; the 64-byte ML-KEM seed), and signing
// uses ML-DSA's DETERMINISTIC variant with an empty context, so every output here is a pure function of its
// inputs and the KAT can pin the derived key + transcript. (Real use feeds those seeds from a CSPRNG; the
// signature itself need not be deterministic for the protocol — the verifier checks it either way, which is
// why Python may sign hedged and Rust deterministically and both still interoperate.)
//
// Three flights for MUTUAL authentication (each side signs a transcript that includes the other's
// contribution — the standard SIGMA/TLS pattern):
//   flight 1  A -> B : dh_i , kem_pk_i           (handled by the caller; consumed by responder_respond)
//   flight 2  B -> A : dh_r , kem_ct , sig_r     (auth_responder_respond)
//   flight 3  A -> B : sig_i                     (auth_initiator_finish -> auth_responder_confirm)
...
/// ML-DSA signing/verifying context. EMPTY to match Python `cryptography`'s default `sign(data)` — the
/// domain separation is carried by the per-role label prepended to the message, not the FIPS context arg.
const MLDSA_CONTEXT: &[u8] = b"";

/// ML-DSA-65 fixed sizes (FIPS 204): 32-byte keygen seed, 1952-byte verifying (public) key, 3309-byte sig.
pub const MLDSA_SEED_LEN: usize = 32;
pub const MLDSA_PUB_LEN: usize = 1952;
pub const MLDSA_SIG_LEN: usize = 3309;

/// Regenerate the ML-DSA-65 signing key from its 32-byte seed (FIPS 204 KeyGen). Proven byte-identical to
/// OpenSSL's `from_seed_bytes`, so the seed is the portable, KAT-pinnable private key (mirror of ml-kem).
fn mldsa_signing_key(seed: &[u8]) -> Option<SigningKey<MlDsa65>> {
    let s = B32::try_from(seed).ok()?; // None on a wrong-length seed (must be exactly 32 bytes)
    Some(SigningKey::<MlDsa65>::from_seed(&s))
}

/// The 1952-byte verifying (public) key for a given seed — a peer's long-term ML-DSA identity. None if the
...
/// Deterministically sign a message under the seed's ML-DSA-65 key (empty context). Returns the 3309-byte
/// encoded signature. None on a wrong-length seed. Deterministic so the KAT/tests are reproducible; a
/// hedged (randomised) signature from Python's OpenSSL verifies identically — verification is variant-blind.
...
/// Verify an ML-DSA-65 signature (empty context). False — never a panic — on any malformed public key or
/// signature, so a tampered/impostor flight fails closed.
pub fn mldsa_verify(public: &[u8], msg: &[u8], sig: &[u8]) -> bool {
    ...
    let sig = match Signature::<MlDsa65>::decode(&enc_sig) {
        Some(s) => s,
        None => return false, // structurally invalid signature encoding
    };
    vk.verify_with_context(msg, MLDSA_CONTEXT, &sig)
}

/// Short human-verifiable fingerprint binding BOTH of a peer's identity keys (ML-DSA public + static DH
/// public), so one out-of-band check covers both (mirror PublicIdentity.fingerprint). 8 bytes = the first
/// 16 hex chars Python prints.
...
/// The handshake transcript (mirror _transcript): SHA-512 over BOTH identities + every public value, in a
/// fixed initiator-then-responder order, each field 8-byte-length-prefixed so no boundary can be slid. Both
/// sides build the identical 64-byte digest; the signatures sign it and the key mixes it, so a tampered
/// value or a swapped identity breaks both (defeats man-in-the-middle, unknown-key-share, identity misbinding).
...
/// The authenticated combiner (mirror _combine): mix the two confidentiality secrets (ee = ephemeral DH,
/// pq = ML-KEM) AND the two static identity-binding terms (es, se — sorted so both roles agree) with the
/// transcript, all length-prefixed. First 32 bytes of SHA-512 are the session key. The key is unknown
/// unless you hold the ephemeral secrets AND can do the static DH, so it survives the loss of either axis.
pub fn auth_combine(
    ee: &[u8],
    pq: &[u8],
    es: &[u8],
    ...
    let (lo, hi) = if es <= se { (es, se) } else { (se, es) }; // sorted(), lexicographic like Python
    let mut h = Sha512::new();
...
/// Responder's flight 2 (mirror Responder.respond): encapsulate to the initiator's ML-KEM key, derive the
/// session key, and sign the full transcript. Returns (our ephemeral DH public `dh_r`, the KEM ciphertext,
/// our ML-DSA signature `sig_r`, the session key, the transcript — kept for confirm()). None on any bad
/// input. All randomness is owned by the caller via the explicit seeds/exponents.
...
pub fn auth_responder_respond(
    ...
    let ee = dh_raw_shared(resp_eph_private, dh_i)?; // eph x peer eph (forward secrecy)
    let es = dh_raw_shared(resp_static_private, dh_i)?; // STATIC x peer eph (identity binding)
    let se = dh_raw_shared(resp_eph_private, init_static_public)?; // eph x peer STATIC (identity binding)
...
/// Initiator's flight 3 (mirror Initiator.finish): VERIFY the responder's signature over the transcript,
/// derive the session key, and produce our own signature `sig_i` to send back. None if the responder's
/// signature does not verify (impersonation/tamper caught — the key is never returned) or on any bad input.
#[allow(clippy::type_complexity, clippy::too_many_arguments)]
pub fn auth_initiator_finish(
    ...
    // Verify the RESPONDER's signature FIRST — refuse (None) before deriving anything if it fails.
    let resp_msg = [AUTH_SIG_CTX_RESPONDER, &transcript[..]].concat();
    if !mldsa_verify(resp_sig_public, &resp_msg, sig_r) {
        return None;
    }
    let ee = dh_raw_shared(init_eph_private, dh_r)?; // eph x peer eph
    let pq = mlkem_decapsulate(init_kem_seed, kem_ct)?;
    let es = dh_raw_shared(init_eph_private, resp_static_public)?; // eph x peer STATIC
    let se = dh_raw_shared(init_static_private, dh_r)?; // STATIC x peer eph
    let key = auth_combine(&ee, &pq, &es, &se, &transcript, info);
...
/// Responder's confirm (mirror Responder.confirm): verify the initiator's signature over the transcript
/// returned by auth_responder_respond. True only if the initiator's identity proof holds — otherwise the
/// caller must discard the session key. Fails closed (false) on any malformed input.
...
mod tests {
    ...
    fn mldsa_sign_verify_and_seed_determinism() {
        // Deterministic keygen: the same seed always yields the same 1952-byte verifying key.
        ...
        // Sign/verify round-trips; deterministic signing gives a stable 3309-byte signature.
        let msg = b"authenticate me";
        let sig = mldsa_sign(&seed, msg).unwrap();
        assert_eq!(sig.len(), MLDSA_SIG_LEN);
        assert_eq!(mldsa_sign(&seed, msg).unwrap(), sig); // deterministic
        assert!(mldsa_verify(&pk, msg, &sig));
        // A flipped message byte, a flipped signature byte, and a different key all fail closed.
    ...
    fn auth_handshake_agrees_and_catches_impersonation() {
        // Long-term identities (ML-DSA seed + static DH) and per-session ephemerals — all caller-pinned.
        ...
        // Published identities + flight 1 (initiator's ephemeral DH public + ML-KEM public).
        ...
        let (dh_r, kem_ct, sig_r, key_b, transcript) = auth_responder_respond(
            &b_sig_seed, &b_static, &b_eph, &a_sig_pub, &a_static_pub, &dh_i, &kem_pk_i, &kem_m, info,
        )
        .unwrap();

        // flight 3: initiator verifies the responder, derives the key, signs back.
        let (key_a, sig_i) = auth_initiator_finish(
            &a_sig_seed, &a_static, &a_eph, &a_kem_seed, &b_sig_pub, &b_static_pub, &dh_r, &kem_ct,
            &sig_r, info,
        )
        .unwrap();
        assert_eq!(key_a, key_b, "authenticated initiator and responder must agree");

        // responder confirms the initiator's signature.
        assert!(auth_responder_confirm(&transcript, &a_sig_pub, &sig_i));

        // Impersonation 1: a tampered responder signature -> initiator refuses (no key returned).
        ...
        // Impersonation 2: a tampered initiator signature -> responder confirm fails.
        let mut bad_sig_i = sig_i.clone();
        bad_sig_i[200] ^= 0x01;
        assert!(!auth_responder_confirm(&transcript, &a_sig_pub, &bad_sig_i));

        // Man-in-the-middle: a tampered ciphertext changes the transcript -> responder sig won't verify.
        ...
        // Fingerprint binds both identity keys and is stable.
